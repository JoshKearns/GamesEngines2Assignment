...
{
    [Header("Spawn Setup")]
    [SerializeField] private bool isCameraFish;
    ...
    private void GenerateUnits()
    {
        ...
        for (int i = 0; i < flockSize; i++)
        {
            ...
            allUnits[i] = Instantiate(flockUnitPrefab[UnityEngine.Random.Range(0, flockUnitPrefab.Length)], spawnPosition, rotation);
            allUnits[i].transform.parent = spawnHolder.transform;
            allUnits[i].AssignFlock(this);
            allUnits[i].InitializeSpeed(UnityEngine.Random.Range(minSpeed, maxSpeed));

            if (isCameraFish)
            {
                allUnits[i].GetComponent<FlockUnit>().SetFishCamSmoothDamp();
                allUnits[i].tag = "FishCam";
                isCameraFish = false;
            }
        }
    }
